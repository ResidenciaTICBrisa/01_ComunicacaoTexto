# Feature de marcação dos parágrafos
A funcionalidade de marcação de páragrafos faz com que o usuário consiga acompanhar em qual parte do texto o leitor se encontra.

### Funcionamento
O usuário ao executar o snippet normalmente já irá ver a função de marcação ativada.
...
A modificação necessária foi realizada na função webSpeech, foram criadas duas funções, uma para marcação e outra para desmarcação após a leitura do texto ser realizada.
```
let readIndex = 0
